...
The 5 outliers in the day-1 samples are 0 counts that were not filtered out by cpm. I cannot justify removing them, since it is possible that there is a systemically underexpressed gene for D1 samples (5 out of 6 D1 samples have this outlier) that is overexpressed in D20 samples.
```{r norm}
...
boxplot(log2(norm_data + 0.0001), ylab = "log2 CPM", main= "Expression distribution, post-normalization", cex = 0.5, cex.axis=0.7, las=2)
```

TMM normalization did not remove the 0-count outliers, and the low-resolution view of the data pre- and post-normalization look very similar.
...
I noticed 5 outliers in my data (the zero-counts in 5 out of 6 D1 samples). I didn't remove any because I felt that their presence was too consistent across one group to simply be measurement error.

*How did you handle replicates?*

I split my data into 4 groups of 3. One group for each combination of time and treatment, and each group contains 3 replicates. This is the same number I use to define the low-count threshold for edgeR.
